                    ...
                    (
                        ...
                        CONSTRAINT `fk_id_teacher`
                    	    FOREIGN KEY (`id_teacher`)
                                REFERENCES `{group_id}`.`Teachers` (`id_teacher`)
                    		    ON DELETE NO ACTION
                    		    ON UPDATE CASCADE
                    )
                    ENGINE = InnoDB;"""
                    ...
                    (
                        ...
                        CONSTRAINT `subject_id fk from Queue to Subjects`
                            FOREIGN KEY (`subject_id`)
                                REFERENCES `{group_id}`.`Subjects` (`subject_id`)
                                ON DELETE CASCADE
                                ON UPDATE CASCADE
                    )
                    ENGINE = InnoDB;"""
                    ...
                    (
                        ...
                        CONSTRAINT `id_queue fk from Sign_ups to Queue`
                            FOREIGN KEY (`id_queue`)
                                REFERENCES `{group_id}`.`Queues` (`id_queue`)
                                ON DELETE NO ACTION
                                ON UPDATE NO ACTION,
                        CONSTRAINT `telegram_user_id fk from Sign_ups to Students`
                            FOREIGN KEY (`telegram_user_id`)
                                REFERENCES `{group_id}`.`Students` (`telegram_user_id`)
                                ON DELETE NO ACTION
                                ON UPDATE NO ACTION
                    )
                    ENGINE = InnoDB;"""
...
def form_system_parameters(group_id):
    if mydb and my_cursor:
        group_password = ''.join(random.choices(string.ascii_uppercase, k=12))
        query = f"INSERT INTO `{group_id}`.`System_settings` (`password`) VALUES ('{group_password}');"
        my_cursor.execute(query)
        mydb.commit()
    else:
        print('\033[91mQueryError: No valid connection to database for commiting a query\033[0m')
...
def delete_database(group_id):
    if mydb and my_cursor:
        ...
        my_cursor.execute(query)
        mydb.commit()
        print(f'\n\033[92mDatabase for group {group_id} was successfully deleted!\033[0m')
    else:
